...
# ----------------------------
if (file.exists(out_file) && !FORCE_FULL_REPULL) {
  ...
  modified_from <- format(last_mod_time + milliseconds(1), "%Y-%m-%dT%H:%M:%OS3Z")
...
get_tests_page <- function(modified_from_utc) {
  ...
  if (sc %in% c(401, 403)) {
    ...
    token <<- get_token()
    resp2 <- GET(url, add_headers(Authorization = paste("Bearer", token)))
