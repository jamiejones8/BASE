...
PROFILES_API_BASE <- paste0("https://prd-", Sys.getenv("VALD_REGION", "use"), "-api-externalprofile.valdperformance.com")
TENANTS_API_BASE  <- paste0("https://prd-", Sys.getenv("VALD_REGION", "use"), "-api-externaltenants.valdperformance.com")
...
if (nrow(csv_groups) > 0) {
  ...
  old_profiles <- safe_read_rds(OUT_PROFILES_RDS)
  if (!is.null(old_profiles) && all(c("profileId", "primaryGroup", "groupNames_csv") %in% names(old_profiles))) {
    idx <- match(profiles_groups$profileId, old_profiles$profileId)
    profiles_groups$groupNames_csv <- dplyr::coalesce(old_profiles$groupNames_csv[idx], "")
    profiles_groups$primaryGroup <- canonical_primary_group(profiles_groups$groupNames_csv,
      dplyr::coalesce(old_profiles$primaryGroup[idx], ""), profiles_groups$externalId)
  }
  cat("[CSV] Retaining known groups where available.\n")
}
