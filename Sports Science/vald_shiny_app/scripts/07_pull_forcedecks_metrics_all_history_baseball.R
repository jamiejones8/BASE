...
# Refetch changed tests, including edits to already downloaded trials.
if (!is.null(existing_final) && "modifiedDateUtc" %in% names(existing_final)) {
  versions <- existing_final %>% group_by(testId) %>% summarise(saved_modified = max(as.character(modifiedDateUtc)), .groups = "drop")
  changed <- force_baseball_tests %>% left_join(versions, by = "testId") %>%
    filter(is.na(saved_modified) | as.character(modifiedDateUtc) != saved_modified) %>% pull(testId)
  existing_test_ids <- setdiff(existing_test_ids, changed)
} else {
  existing_test_ids <- character(0)
}
...
trials_url_for <- function(test_id) {
  paste0(
    paste0("https://prd-", Sys.getenv("VALD_REGION", "use"), "-api-extforcedecks.valdperformance.com"),
...
if (length(test_ids) > 0 && file.exists(partial_list_file)) {
  ...
  if (is.list(partial_state) && identical(partial_state$test_ids, test_ids)) {
    ...
    missing_idx <- which(vapply(all_metrics, is.null, logical(1)))
    start_i <- if (length(missing_idx)) min(missing_idx) else length(test_ids) + 1L
...
# ----------------------------
if (length(test_ids) && any(vapply(all_metrics, is.null, logical(1)))) {
  stop("Some trial requests failed or returned no trials; retaining published data and retrying on the next refresh.")
}
if (!is.null(existing_final) && nrow(new_metrics_joined)) {
  existing_final <- existing_final %>% filter(!testId %in% new_metrics_joined$testId)
}
metrics_long_joined <- dplyr::bind_rows(new_metrics_joined, existing_final) %>%
