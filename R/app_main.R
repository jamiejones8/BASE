base_bootstrap_file <- local({
  ...
  if (length(frame_files)) frame_files[[length(frame_files)]] else normalizePath("R/app_main.R", winslash = "/", mustWork = FALSE)
})

base_bootstrap_root <- normalizePath(
  file.path(dirname(base_bootstrap_file), ".."),
  winslash = "/",
  mustWork = TRUE
)
...
pitcher_summary <- function(tmilb){
  tmilb %>%
    dplyr::select(dplyr::any_of(c("Date","GameUID","HomeTeam","AwayTeam","BatterTeam","Tilt","ExitSpeed")),
    ...
    dplyr::mutate("Hit" = dplyr::case_when(PlayResult %in% c("Single","Double","Triple","HomeRun") ~ TRUE,TRUE ~ FALSE),
         "CallStrike" = dplyr::case_when(PitchCall %in% c("StrikeCalled") ~ TRUE, TRUE ~ FALSE),
         "Whiff" = dplyr::case_when(PitchCall %in% c("StrikeSwinging") ~ TRUE, TRUE ~ FALSE),
         "CSW" = CallStrike + Whiff,
         "Contact" = dplyr::case_when(PitchCall %in% c("FoulBall","FoulBallNotFieldable","InPlay") ~ TRUE, TRUE ~ FALSE),
         "GB" = dplyr::case_when(TaggedHitType %in% c('GroundBall') ~ TRUE, TRUE ~ FALSE),
         "LD" = dplyr::case_when(TaggedHitType %in% c('LineDrive') ~ TRUE, TRUE ~ FALSE),
         "FB" = dplyr::case_when(TaggedHitType %in% c ("FlyBall") ~ TRUE, TRUE ~ FALSE),
         "PopU" = dplyr::case_when(TaggedHitType %in% c ("Popup") ~ TRUE, TRUE ~ FALSE),
         "Swing" = Whiff + Contact,
         "BBE" = GB + LD + FB + PopU,
        # "HardHit" = ifelse(ExitSpeed >= 95,TRUE,FALSE),
         "Ball" = dplyr::case_when(PitchCall %in% c("BallCalled","BallinDirt") ~ TRUE, TRUE ~ FALSE),
         "Single" = dplyr::case_when(PlayResult %in% c("Single") ~ TRUE, TRUE ~ FALSE),
         "Double" = dplyr::case_when(PlayResult %in% c("Double") ~ TRUE, TRUE ~ FALSE),
         "Triple" = dplyr::case_when(PlayResult %in% c("Triple") ~ TRUE, TRUE ~ FALSE),
         "HR" = dplyr::case_when(PlayResult %in% c("HomeRun") ~ TRUE, TRUE ~ FALSE),
         "Sac" = dplyr::case_when(PlayResult %in% c("Sacrifice") ~ TRUE, TRUE ~ FALSE),
         "HBP" = dplyr::case_when(PitchCall %in% c("HitByPitch") ~ TRUE, TRUE ~ FALSE),
         "Error" = dplyr::case_when(PlayResult %in% c("Error") ~ TRUE, TRUE ~ FALSE),
         "FC"= dplyr::case_when(PlayResult %in% c("FieldersChoice") ~ TRUE, TRUE ~ FALSE),
         "Out" = dplyr::case_when(PlayResult %in% c ("Out") ~ TRUE, TRUE ~ FALSE),
        ...
        #  "PSituation" = ifelse(Balls == Strikes,"Even",PSituation),
         ...
         "Zone" = dplyr::case_when(dplyr::between(PlateLocSide,-.825,.825) & dplyr::between(PlateLocHeight,1.45,3.45) ~ TRUE, TRUE ~ FALSE),
...
left_batter <- png::readPNG(TEAM_CONFIG$data$left_batter_image_file)
right_batter <- png::readPNG(TEAM_CONFIG$data$right_batter_image_file)
